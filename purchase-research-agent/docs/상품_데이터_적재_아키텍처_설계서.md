...
> 이 문서는 `docs/AI 구매 Agent 상품 데이터 수집 시스템 개발 계획서.pdf`, `docs/AI 구매 Agent 상품 수집 시스템.pdf`(범용 다중 쇼핑몰 크롤링 시스템 샘플)를 참고하여, **실제로 진행 중인 이 프로젝트**(ABC마트 전용 Python 크롤러 + Spring Boot 백엔드)에 맞게 축소·구체화한 문서다.
>
...
purchase-research-agent (Python)
  ├─ AbcMartCrawler: 상품 리스트 크롤링
  ├─ DetailFetcher: 리뷰/옵션 수집
  ├─ (정제) 가격 유무 필터링 등
  └─ HTTP POST ──────────────────┐
                                  ▼
