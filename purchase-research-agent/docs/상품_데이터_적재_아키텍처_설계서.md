...
| 조건 매핑 모호성 | 자연어 조건(가격/카테고리)을 SQL WHERE로 어떻게 매핑할지 미정 | **1차 구현 완료**: `query_parser.py`가 정규식으로 가격만 뽑고 나머지 텍스트를 `title LIKE` 키워드로 그대로 씀(§10.4). 카테고리 컬럼 자체가 DB에 없어서 "카테고리"가 아니라 "제목에 포함된 단어" 수준의 매칭임 |
...
7. ✅ §3.3 조회/추천 파이프라인 1차 구현 (CLI, Spring 조회/트리거 API, 규칙 기반 Query/Recommendation Agent) — 상세는 §10.4
8. `[TBD]` 규칙 기반 Query/Recommendation Agent를 실제 LLM 호출로 교체
...
- [app/services/query_parser.py](../app/services/query_parser.py) — Query Analysis Agent 임시 구현(규칙 기반): "N만원 이하/미만" 정규식으로 가격 추출, 나머지 텍스트를 키워드로
- [app/services/recommender.py](../app/services/recommender.py) — Recommendation Agent 임시 구현(규칙 기반): 할인율 내림차순 → 가격 오름차순 정렬 후 상위 5개
- [cli.py](../cli.py) — §3.3 CLI 진입점. 입력 → 분석 → Spring 조회 → (필요시) 크롤링 트리거 → 재조회 → 추천 출력
...
- `dto/` — `ProductBatchRequest`, `ProductPayload`, `OptionsPayload`, `ReviewPayload`, `ProductBatchResponse`(+`FailureItem`), `ProductSearchResponse`(+`ProductSummary`), `FastApiProductItem`, `FastApiSearchResponse`(FastAPI 응답 snake_case를 매핑하는 전용 DTO)
...
- [x] CLI Client — [cli.py](../cli.py) (`python cli.py`로 실행, 자연어 입력 → 결과 출력)
- [x] Query Analysis Agent — [app/services/query_parser.py](../app/services/query_parser.py). **규칙 기반 임시 구현**(정규식으로 "N만원 이하/미만" 가격, 나머지 텍스트를 키워드로 추출). LLM 아님
- [x] Spring 조회(SELECT) API — `GET /api/v1/products/search?keyword=&maxPrice=` ([ProductQueryService.java](../../purchase-research-backend/src/main/java/com/purchaseresearch/backend/service/ProductQueryService.java), title LIKE + price 이하 조건)
- [x] 임계값(10건) 판단 로직 — Spring이 아니라 **CLI(Python) 쪽에 둠** (§16 결정, 아래 참고)
- [x] Recommendation Agent — [app/services/recommender.py](../app/services/recommender.py). **규칙 기반 임시 구현**(할인율 높은 순 → 가격 낮은 순 정렬, 상위 5개). LLM 아님
- [x] Spring → Python 크롤링 트리거(pull) — `POST /api/v1/products/crawl-trigger` ([CrawlTriggerService.java](../../purchase-research-backend/src/main/java/com/purchaseresearch/backend/service/CrawlTriggerService.java)): FastAPI `/api/v1/search`를 호출해서 크롤링시키고, 받은 결과를 같은 요청 안에서 바로 `ProductIngestService`로 적재까지 함(별도 `/batch` 왕복 없음)
- [ ] 비동기 완료 알림/폴링 (§3.3, §9) — **미구현**. 지금은 CLI가 크롤링 끝날 때까지 동기로 기다림(§12 결정 항목에 사유 정리)
...
| 단계 | 내용 | 증거 |
|---|---|---|
| Spring → FastAPI 요청 | `CrawlTriggerService.triggerAndIngest()`가 `crawlerServiceRestClient.post().uri("/api/v1/search"...)`로 실제 호출 | **FastAPI 서버 자체 접속 로그**: `POST /api/v1/search?keyword=%EB%B6%80%EC%B8%A0&site=abcmart&max_items=30 HTTP/1.1" 200 OK` |
| FastAPI가 실제 ABC마트를 크롤링 | [crawler.py](../app/crawlers/abcmart/crawler.py)의 `AbcMartCrawler.crawl()` | `[FETCH]... https://abcmart.a-rt.com/...`, `[ABCMART:search] page=1 +30개`, `[RAW] 30개 -> output\raw\abcmart_부츠_raw_20260726_192112.json`(실제 파일 존재) |
| FastAPI → Spring 응답 성공 | 위와 같은 요청의 응답 | 같은 로그 라인의 `200 OK` |
| Spring → MySQL 적재 | `ProductIngestService.ingest()` | **MySQL 직접 SELECT로 확인**: 상품 14건, `updated_at = 2026-07-26 19:21:25`(요청 시각과 일치), `price_history` 20행 누적 |

**로그는 없지만 코드 흐름상 확실한 단계** (별도 프로세스 간 호출이 아니라서 독립 로그가 안 남음):

| 단계 | 왜 로그가 없나 |
|---|---|
| `cli.py` → Spring `GET /search`, `POST /crawl-trigger` 호출 | Spring에 요청 접근 로그(access log)를 별도로 설정 안 함 — Spring이 요청을 받았다는 걸 Spring 쪽 로그로는 못 봄 |
| FastAPI 내부 `search.py` → `crawler_service.py` 함수 호출 | 같은 파이썬 프로세스 안의 함수 호출이라 별도 로그 없음 |
| Spring `CrawlTriggerService` → `ProductIngestService.ingest()` | 같은 JVM 안의 자바 메서드 직접 호출(HTTP 아님)이라 로그 없음. 결과물(MySQL 적재)로 간접 증명됨 |
| Spring → `cli.py` 최종 응답 | `cli.py`가 콘솔에 찍은 텍스트뿐이라, 그 자체의 독립 증거는 아님(다만 그 다음 재조회에서 `totalCount`가 실제로 늘어난 것이 간접 증거) |
...
12. ~~비동기 전환 시점~~ → **처음(CLI 단계)부터 비동기로 설계** (§3.3, §9). ⚠️ 다만 §10.4에서 실제로 만든 1차 구현은 **동기**다(CLI가 크롤링 끝날 때까지 기다림) — 비동기 알림 방식(§17)이 아직 없어서 임시로 동기로 연결해둔 상태. 방향 결정을 뒤집은 게 아니라, §17이 풀리기 전까지의 잠정 구현이다.
13. ~~Query Analysis Agent / Recommendation Agent를 어느 프로세스에 둘 것인가~~ → **Python(CLI와 같은 프로세스, `app/services/query_parser.py`/`recommender.py`)에 둠.** 단, LLM 호출이 아니라 **규칙 기반 임시 구현**이다(`ANTHROPIC_API_KEY` 없어서). LLM로 교체하는 것은 별도 과제로 남음(§10.4)

### 아직 남은 질문

14. 크롤링 실패/부분 성공을 상품 데이터에 플래그로 남길 것인가? (§9)
15. 재고(품절 여부) 수집 및 `stock_history` 도입 여부 (§7.2)
...
19. 자연어 요청 → SQL 조건 매핑 규칙을 정교화할 것인가 (지금은 title LIKE + 가격 이하뿐, 브랜드/할인율 등 조건 추가 필요할지), 임계값(10건)을 카테고리별로 다르게 둘지 (§3.3, §9)
...
22. 배치 분할 호출 중 일부 청크만 실패했을 때 나머지를 계속 보낼지 (§6.1)
