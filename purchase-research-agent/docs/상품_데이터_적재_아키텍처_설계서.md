...
> **구현 상태: 설계만 완료, 코드는 없음.** 이 절에서 "결정했다"는 표현은 전부 "이렇게 만들기로 방향을 정했다"는 뜻이지, 실제로 구현됐다는 뜻이 아니다. CLI Client, Query Analysis Agent, Spring의 조회(SELECT) API, 임계값 판단 로직, Recommendation Agent, 크롤링 트리거(pull) 호출, 비동기 완료 알림 — 전부 코드가 하나도 없다. 지금까지 실제로 만든 건 §3.1 수집 파이프라인(크롤링 → 파싱 → 적재)뿐이다. 구현 체크리스트는 §10.4 참고.
...
### 10.4 조회/추천 파이프라인(§3.3) 구현 체크리스트 — 전부 미착수

§3.1(수집 파이프라인)과 달리, 아래는 설계만 돼 있고 코드가 하나도 없다.

- [ ] CLI Client (사용자 입력을 받는 진입점 자체가 없음)
- [ ] Query Analysis Agent (자연어 → 조회 조건 변환, LLM 연동 없음)
- [ ] Spring 조회(SELECT) API (`POST /api/v1/products/batch`만 있고, 조건으로 상품을 검색해 돌려주는 GET/조회 엔드포인트 없음)
- [ ] 임계값(10건) 판단 로직
- [ ] Recommendation Agent (후보 상품 분석/추천)
- [ ] Spring → Python 크롤링 트리거(pull) — `CrawlerServiceClientConfig`(RestClient 빈)만 스캐폴딩돼 있고 실제로 호출하는 컨트롤러/서비스 없음
- [ ] 비동기 완료 알림/폴링 (§3.3, §9)
