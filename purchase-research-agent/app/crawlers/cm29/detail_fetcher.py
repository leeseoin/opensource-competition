...
def _parse_reviews(raw_results: list[dict], review_limit: int) -> list[dict]:
    reviews = []
    for rv in raw_results[:review_limit]:
        # 이미지 URL (상대경로 → 절대경로)
        images = [
            ...
            for i, product in enumerate(products[:limit]):
                ...
                try:
                    rr = await review_client.get(
                        _REVIEW_URL,
                        params={"itemId": item_id, "page": 1, "size": review_limit},
                    )
                    rv_data = rr.json().get("data", {})
                    product["reviews"] = _parse_reviews(rv_data.get("results", []), review_limit)
                    # rating/review_count를 리뷰 API 값으로 보정 (JSON-LD보다 정확)
                    if rv_data.get("count") is not None:
                        product["review_count"] = rv_data["count"]
