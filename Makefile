...
WEB_PORT ?= 3000
COLLECTOR_HTTP_ADDRESS := $(if $(strip $(COLLECTOR_HTTP_ADDRESS)),$(COLLECTOR_HTTP_ADDRESS),:8090)
COLLECTOR_READ_TIMEOUT := $(if $(strip $(COLLECTOR_READ_TIMEOUT)),$(COLLECTOR_READ_TIMEOUT),5s)
COLLECTOR_WRITE_TIMEOUT := $(if $(strip $(COLLECTOR_WRITE_TIMEOUT)),$(COLLECTOR_WRITE_TIMEOUT),90s)
COLLECTOR_IDLE_TIMEOUT := $(if $(strip $(COLLECTOR_IDLE_TIMEOUT)),$(COLLECTOR_IDLE_TIMEOUT),60s)
COLLECTOR_SHUTDOWN_TIMEOUT := $(if $(strip $(COLLECTOR_SHUTDOWN_TIMEOUT)),$(COLLECTOR_SHUTDOWN_TIMEOUT),10s)
COLLECTOR_WORKER_TIMEOUT := $(if $(strip $(COLLECTOR_WORKER_TIMEOUT)),$(COLLECTOR_WORKER_TIMEOUT),90s)
...
test: collector-test product-backend-test mcp-server-test web-test web-lint ## Go, Spring Boot, MCP, Next.js를 검증한다.
