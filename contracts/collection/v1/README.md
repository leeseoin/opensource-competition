...
Python Research Backend와 Go Collector Worker가 RabbitMQ에서 주고받는 작업과
결과의 공통 JSON 형식을 정의한다.
...
- 임의 URL: 작업에 받지 않으며 Go 판매처 Adapter가 공개 수집 URL을 생성

`maxAttempts=2`는 최초 처리 한 번과 일시 오류 재시도 한 번을 뜻한다. 로그인,
접근 차단, 지원하지 않는 판매처와 잘못된 계약은 재시도하지 않는다.
...
Python은 판매처, operation, 검색 payload를 정렬한 JSON으로 만든 뒤 SHA-256을
계산한다.
...
현재 메시지에는 키를 기록하고, 실제 중복 등록 차단은 다음 Redis adapter 작업에서
구현한다.
