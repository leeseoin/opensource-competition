...
작성일: 2026-07-31
최종 점검일: 2026-08-10
최종 점검 범위: `QUEUE-001`, `QUEUE-002` 계약 실패 복구와 실제 판매처 Queue E2E
...
| 기능 ID | 기능명 | 상태 | 우선순위 | 범위 | 완료 기준 | 설계 근거 | 다음 작업 |
|---|---|---|---|---|---|---|---|
| `QUEUE-001` | 검색 작업 계약과 Collector Worker | 완료 | P0 | RabbitMQ topology, 작업/결과 계약, Python/Go 소비, retry와 DLQ | fixture 작업의 성공/실패/재시도/ACK/DLQ 통합 테스트가 통과함 | 시스템 구조 / 데이터 수집과 DB 적재 설계 | 비정상 종료 뒤 미확인 작업 복구 검증은 운영 안정성 후속 작업으로 유지 |
| `QUEUE-002` | Spring Boot 작업 발행과 결과 저장 | 완료 | P0 | Product Backend producer, 결과 consumer, 검증 실패 처리와 DB transaction 연결 | Product Backend가 작업을 발행하고 Collector 결과를 검증해 PostgreSQL에 저장함 | 시스템 구조 / 현재 DB 저장 흐름 | Worker 또는 Backend 비정상 종료 복구는 운영 안정성 후속 작업으로 유지 |
