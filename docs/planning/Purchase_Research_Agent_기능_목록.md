...
| `QUEUE-002` | Spring Boot 작업 발행과 결과 저장 | 계획 | P0 | Product Backend producer, 결과 consumer, 검증 실패 처리와 DB transaction 연결 | Product Backend가 작업을 발행하고 Go 결과를 검증해 PostgreSQL에 저장함 | 시스템 구조 / 현재 DB 저장 흐름 | 결과 consumer부터 구현 |
...
1. `QUEUE-002` Spring Boot 결과 consumer
2. `BACKEND-001` 실제 Queue 결과 PostgreSQL 적재 E2E
