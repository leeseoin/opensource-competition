...
작성일: 2026-07-31
최종 점검일: 2026-08-03
최종 점검 범위: 아키텍처 기반 `OPS-004` 신규 등록
상태: 선택 기능 감사 완료
...
| `MCP-002` | 구매 조사 Agent 실행 경계 | 부분 구현 | P1 | Codex Plugin workflow, 질문 구조화, 사용자 조건 확인, 도구 호출과 답변 근거 연결 | Plugin이 판매처나 DB를 직접 호출하지 않고 MCP 도구로 근거 있는 답변을 구성함 | 시스템 구조 | stream/취소/동시성 상한과 Plugin 로컬 설치 검증 |
...
| `OPS-004` | Python/Go 크롤러 확장성과 성능 비교 | 부분 구현 | P1 | 동일 비교 Contract, 최대 10,000개 pagination/checkpoint 수집, parser 및 E2E benchmark | Python/Go가 같은 fixture 계약을 통과하고 판매처별 실제 수집량/중복/오류/시간/CPU/메모리 보고서를 재현함 | [Python/Go 크롤러 비교 설계](../architecture/Python_Go_크롤러_확장성과_성능_비교_설계.md) | Python 최대 10,000개 단계와 Go Adapter/수집 코어 구현 |
