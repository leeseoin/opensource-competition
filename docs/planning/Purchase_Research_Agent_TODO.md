...
| Phase 5 | `MCP-001`, `MCP-002`, `MCP-003` |
...
만들지 않으며 웹과 MCP가 같은 상태와 근거를 표시한다.
...
- [x] Radix UI 기반 공통 select/dropdown과 focus/open/disabled 상태 및 Portal hover 대비 구성
...
- [ ] 최소 1,000개 질문 평가 suite 구축 **(진행 중: 현재 DB 구매 가능 snapshot 기반 1,000개 회귀와 CSV 완료, 고정 fixture와 사람 검토 gold relevance 남음)**
- [ ] 1,000개 질문의 필수 조건 위반율/Recall@20/nDCG@5/false zero/상품군 중복률 자동 집계 **(진행 중: 필수 위반/false zero/상품군 적중/중복/p95 자동 집계 완료, Recall@20과 nDCG@5 gold 판정 남음)**
